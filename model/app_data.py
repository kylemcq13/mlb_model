...
df3 = utils.player_id(df=df2, player_lookup=player_lookup, pitching=pitching, hitting=hitting)
...
df3_2021_reco = df3[['batter_name', 'pitcher_name', 'pitcher_team', 'matchup_score', 'isStarter']].merge(pitcher_df, on='pitcher_name', how='inner').drop(columns=['pitcher_team_x', 'game_date']).rename(columns = {'pitcher_team_y':'pitcher_team'}).drop_duplicates()
